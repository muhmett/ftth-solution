 ...
 * l'organisation interne d'un sous-traitant (ses équipes, ses intervenants)
 * ne lui est pas transmise. Utilisé pour filtrer les réponses avant envoi.
 */
export function hideTeamIdentity(user) {
  return isPercer(user);
}

// Retire d'un objet les champs qui désignent une équipe ou ses intervenants
export function stripTeamFields(user, row) {
  if (!row || !hideTeamIdentity(user)) return row;
  const { equipe_name, equipe_phone, member1, member2, ...rest } = row;
  return rest;
}
