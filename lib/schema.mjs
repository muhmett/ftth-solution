...
// Colonnes ajoutées après coup : appliquées en ALTER TABLE sur les bases existantes
export const ADDED_COLUMNS = [
  ['tickets', 'deadline', 'TEXT'],
];
...
const SIDE_TABLES = `
  ...
  -- Conditions contractuelles par sous-traitant et par activité :
  -- prix de l'intervention, délai accordé, retenue appliquée en cas de retard.
  CREATE TABLE IF NOT EXISTS contract_terms (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    org_id INTEGER NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    ticket_type TEXT NOT NULL CHECK (ticket_type IN ('FTTH','PARTAGE_IN','PARTAGE_OUT','SAV')),
  ...
  -- Réglages techniques persistants (clés VAPID des notifications, ...)
  CREATE TABLE IF NOT EXISTS app_settings (
    key TEXT PRIMARY KEY,
    value TEXT NOT NULL
  );

  -- Catalogue matériel, tenu par Percer : la même référence circule dans toute
  -- la chaîne (PTO, ONT, câble, connecteurs...).
  ...
  -- Journal des mouvements : le stock n'est jamais stocké, il se recalcule.
  -- Deux emplacements par société : le dépôt (equipe_id NULL) et le véhicule
  -- de chaque équipe.
  CREATE TABLE IF NOT EXISTS stock_movements (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    org_id INTEGER NOT NULL REFERENCES organizations(id),
    material_id INTEGER NOT NULL REFERENCES materials(id),
    kind TEXT NOT NULL CHECK (kind IN ('ENTREE','ATTRIBUTION','CONSOMMATION','RETOUR','PERTE','AJUSTEMENT')),
    quantity REAL NOT NULL,
    equipe_id INTEGER REFERENCES users(id),
...
const INDEXES = `
  ...
  CREATE INDEX IF NOT EXISTS idx_stock_org ON stock_movements(org_id, material_id);
  CREATE INDEX IF NOT EXISTS idx_stock_equipe ON stock_movements(equipe_id);
  CREATE INDEX IF NOT EXISTS idx_stock_ticket ON stock_movements(ticket_id);
`;
